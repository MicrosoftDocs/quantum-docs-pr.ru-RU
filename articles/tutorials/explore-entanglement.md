...
Первое, что нужно сделать, — это создать новый Q# проект. В этом учебнике мы будем использовать среду на основе [приложений командной строки с VS Code](xref:microsoft.quantum.install.standalone).
...
Возвращаемый тип операции задается после двоеточия. В нашем примере операция `SetQubitState` ничего не возвращает, поэтому для нее указан возвращаемый тип `Unit`. Это Q# эквивалент `unit` в F #, который примерно аналогичен `void` в C#, и пустой кортеж ( `Tuple[()]` ) в Python.
...
Чтобы выполнить код, необходимо указать компилятор *,* вызываемый для запуска при предоставлении `dotnet run` команды. Это делается простым изменением в Q# файле путем добавления строки, `@EntryPoint()` непосредственно предшествующей вызываемой: в `TestBellState` этом случае операция. Полный код должен быть следующим:
