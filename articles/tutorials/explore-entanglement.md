...
Вы также можете следовать инструкциям, не устанавливая КДК, просматривая обзор языка программирования Q # и первых концепций тактовых вычислений.
...
> * Демонстрация тактовой замкнутые с программой Q #, выполняемой в симуляторе. 
...
## <a name="creating-a-q-project"></a>Создание проекта Q #

Первое, что нужно сделать, — это создать новый проект Q #. В этом учебнике мы будем использовать среду на основе [приложений командной строки с VS Code](xref:microsoft.quantum.install.standalone).
...
В этом случае мы назвали проект `Bell` . Это приводит к созданию двух файлов: `Bell.csproj` , файла проекта и `Program.qs` шаблона приложения Q #, которое будет использоваться для написания нашего приложения. Содержимое `Program.qs` должно быть следующим:
...
Мы намерены подготовить два кубита в особом квантовом состоянии, на примере которых мы продемонстрируем работу с кубитами в Q#, изменяя их состояние и применяя эффекты суперпозиции и запутанности. Мы создадим этот фрагмент по части для представления кубитных состояний, операций и измерений.

### <a name="initialize-qubit-using-measurement"></a>Инициализация кубит с помощью измерения

В первом примере кода мы покажем, как работать с кубитами в Q#.  Мы предоставим две операции, [`M`](xref:microsoft.quantum.intrinsic.m) [`X`](xref:microsoft.quantum.intrinsic.x) которые преобразуют состояние кубит. В этом фрагменте кода определена операция `SetQubitState`, которая принимает в качестве параметров кубит и значение состояния `desired`, в котором должен находиться этот кубит.  Операция `SetQubitState` выполняет измерение кубита с помощью операции `M`.  В Q # кубит измерение всегда возвращает значение `Zero` или `One` .  Если измерение возвращает значение, не равное желаемому значению, `SetQubitState` "переворачивает" кубит, то есть выполняет `X` операцию, которая изменяет состояние кубит на новое состояние, в котором вероятности возврата и отмены измерений отменяются `Zero` `One` . Таким образом `SetQubitState` Целевая кубит всегда помещается в нужное состояние.
...
Операция `SetQubitState` измеряет кубит. Если кубит находится в нужном состоянии, `SetQubitState` больше не влияет на него, в противном случае мы выполняем операцию `X`, чтобы изменить состояние кубита на требуемое.

#### <a name="about-q-operations"></a>Сведения об операциях Q#

Операцией в Q# называется квантовая подпрограмма. То есть это вызываемая подпрограммы, которая содержит вызовы других тактовых операций.

Аргументы операции задаются в виде кортежа в круглых скобках.

Возвращаемый тип операции задается после двоеточия. В нашем примере операция `SetQubitState` ничего не возвращает, поэтому для нее указан возвращаемый тип `Unit`. Эта конструкция Q# эквивалентна `unit` в F# и похожа на `void` в C# или пустой кортеж (`Tuple[()]`) в Python.
...
В файле `Bell.qs` добавьте следующую операцию в пространство имен, сразу после операции `SetQubitState`:
...
Если же вам нужна переменная, значение которой может меняться (в нашем примере это `numOnes`), объявите такую переменную с ключевым словом `mutable`. Значение изменяемой переменной можно изменить с помощью инструкции `setQubitState`.
...
## <a name="execute-the-code-from-the-command-line"></a>Выполнение кода из командной строки

Чтобы выполнить код, необходимо указать компилятор *,* вызываемый для запуска при предоставлении `dotnet run` команды. Это делается простым изменением в файле Q # путем добавления строки, `@EntryPoint()` непосредственно предшествующей вызываемой функцией: в `TestBellState` этом случае — операция. Полный код должен быть следующим:
