...
Возвращаемый тип операции задается после двоеточия. В нашем примере операция `SetQubitState` ничего не возвращает, поэтому для нее указан возвращаемый тип `Unit`. Это Q# эквивалент `unit` в F #, который примерно аналогичен `void` в C#, и пустой кортеж в Python ( `()` представленный указанием типа `Tuple[()]` ).
