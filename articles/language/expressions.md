...
При наличии целочисленного или длинного целочисленного выражения слева, а также целочисленного выражения справа можно использовать операторы `<<<` (арифметические сдвиги влево) или `>>>` (арифметические сдвиги вправо), чтобы создать новое выражение с тем же типом, что и в левой части выражение.
...
Выражение будет иметь значение true, если два выражения (отв. не) равны.

Значения определяемых пользователем типов могут не сравниваться, поэтому можно сравнивать только их значения. Например,
...
Например, если `Func` имеет `('T1, 'T2, 'T1) -> 'T2`сигнатуры, `Op1` и `Op2` имеют сигнатуру `(Qubit[] => Unit is Adj)`, а `Op3` имеет `(Qubit[] => Unit)`сигнатуры, для вызова `Func` с `Op1` в качестве первого аргумента `Op2` в качестве второго. и `Op3` в качестве третьего:
...
operator | Арность | Описание | Типы операндов
---------|----------|---------|---------------
 Конечная `!` | Унар | разворачивание; | Любой определяемый пользователем тип
 `-`, `~~~`, `not` | Унар | Числовое отрицательное, побитовое дополнение, логическое отрицание | `Int`, `BigInt` или `Double` для `-`, `Int` или `BigInt` `~~~`, `Bool`
 `^` | Binary | Целочисленное энергопотребление | `Int` или `BigInt` для основания, `Int` для экспоненты
 `/`, `*`, `%` | Binary | Деление, умножение, целочисленный модуль | `Int`, `BigInt` или `Double` для `/` и `*`, `Int` или `BigInt` для `%`
 `+`, `-` | Binary | Сложение или объединение строк и массивов, вычитание | `Int`, `BigInt` или `Double`, кроме `String` или любого типа массива для `+`
 `<<<`, `>>>` | Binary | Сдвиг влево, сдвиг вправо | `Int` или `BigInt`
 `<`, `<=`, `>`, `>=` | Binary | Сравнения "меньше чем", "меньше или равно", "больше чем", "больше или равно" | `Int`, `BigInt` или `Double`
 `==`, `!=` | Binary | сравнения «равно» и «не равно» | любой тип-примитив
 `&&&` | Binary | Побитовое и | `Int` или `BigInt`
 `^^^` | Binary | Побитовое ИСКЛЮЧАЮЩее | `Int` или `BigInt`
 <code>\|\|\|</code> | Binary | Побитовое или | `Int` или `BigInt`
 `and` | Binary | Логическое и | `Bool`
 `or` | Binary | Логическое или | `Bool`
