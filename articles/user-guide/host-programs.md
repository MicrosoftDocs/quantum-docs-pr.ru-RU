...
Для этого он должен состоять из тела [операции](xref:microsoft.quantum.guide.basics#q-operations-and-functions), которая затем выполняется при вызове---либо непосредственно, либо с помощью другой операции. Таким образом, можно написать операцию следующего вида:
```qsharp
