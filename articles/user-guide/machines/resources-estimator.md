...
|__Глубина__|Глубина тактовой цепи, выполняемой Q# операцией (см. [ниже](#depth-width-and-qubitcount)). По умолчанию метрика глубины подсчитывает только `T` шлюзы. Дополнительные сведения см. в разделе [счетчик глубины](xref:microsoft.quantum.machines.qc-trace-simulator.depth-counter).   |
|__Width__|Ширина тактовой цепи, выполняемой Q# операцией (см. [ниже](#depth-width-and-qubitcount)). По умолчанию метрика глубины подсчитывает только `T` шлюзы. Дополнительные сведения см. в разделе [счетчик глубины](xref:microsoft.quantum.machines.qc-trace-simulator.depth-counter).   |
...
__Глубина:__ Для корневой операции времени, которое требуется для выполнения, предполагается определенное время шлюза.
