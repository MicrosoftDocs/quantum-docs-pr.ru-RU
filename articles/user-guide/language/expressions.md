...
Однако, хотя `(Qubit[] => Unit is Adj)` `(Qubit[] => Unit is Ctl)` операции и имеют общий базовый тип `(Qubit[] => Unit)` , обратите внимание, что *of* массивы этих операторов не имеют общего базового типа. Например, `[[Op1], [Op2]]` в настоящее время вызовет ошибку, так как пытается создать массив несовместимых типов массивов `(Qubit[] => Unit is Adj)[]` и `(Qubit[] => Unit is Ctl)[]` .
...
---------|----------|---------|---------------
 ...
 `^` | Двоичный | Целочисленное энергопотребление | `Int`или `BigInt` для основания, `Int` для показателя степени
 `/`, `*`, `%` | Двоичный | Деление, умножение, целочисленный модуль | `Int`, `BigInt` или `Double` для `/` и `*` , `Int` или `BigInt` для`%`
 `+`, `-` | Двоичный | Сложение или объединение строк и массивов, вычитание | `Int`, `BigInt` или `Double` , Кроме того, `String` или любого типа массива для`+`
 `<<<`, `>>>` | Двоичный | Сдвиг влево, сдвиг вправо | `Int` или `BigInt`
 `<`, `<=`, `>`, `>=` | Двоичный | Сравнения "меньше чем", "меньше или равно", "больше чем", "больше или равно" | `Int`, `BigInt` или`Double`
 `==`, `!=` | Двоичный | сравнения «равно» и «не равно» | любой тип-примитив
 `&&&` | Двоичный | Побитовое И | `Int` или `BigInt`
 `^^^` | Двоичный | Побитовое исключающее ИЛИ | `Int` или `BigInt`
 <code>\|\|\|</code> | Двоичный | Побитовое ИЛИ | `Int` или `BigInt`
 `and` | Двоичный | Логическое И | `Bool`
 `or` | Двоичный | Логическое ИЛИ | `Bool`
