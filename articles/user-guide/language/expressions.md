...
---
# <a name="expressions-in-no-locq"></a>Выражения в Q#
...
Целочисленный разделитель и модуль целых чисел соответствуют тому же поведению для отрицательных чисел, что и в C#. То есть `a % b` всегда имеет тот же знак, что `a` и, и `b * (a / b) + a % b` всегда равно `a` . Пример:
...
## <a name="user-defined-type-expressions"></a>Выражения типа User-Defined
