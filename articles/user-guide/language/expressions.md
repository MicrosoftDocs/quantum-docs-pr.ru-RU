...
Синтаксис Q # для интерполяции строк представляет собой подмножество синтаксиса C#. Ниже приведены основные моменты, которые относятся к Q #:
...
* Выражения внутри строки с интерполяцией следуют синтаксису Q #, а не синтаксису C#. Самое важное отличие заключается в том, что Q # не поддерживает буквальные строки с интерполяцией (многострочные).
...
Так как все типы Q # являются типами значений (с Кубитс, принимающей несколько специальных ролей), формально «Copy» создается при привязке значения к символу или при повторной привязке символа. Это означает, что поведение Q # аналогично тому, как если бы копия была создана с помощью оператора присваивания. 
...
Это действие обычно не требуется, так как компилятор Q # выводит фактические типы.
...
Операторы Q # в порядке приоритета, от самого высокого до самого низкого:
...
Теперь, когда вы можете работать с выражениями в Q #, переходите к [операциям и функциям в q #](xref:microsoft.quantum.guide.operationsfunctions) , чтобы узнать, как определять и вызывать операции и функции.
