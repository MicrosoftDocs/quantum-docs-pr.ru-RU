...
---
# <a name="types-in-no-locq"></a>Типы в Q#

В этой статье описывается Q# модель типов и синтаксис для указания типов и работы с ними. Дополнительные сведения о создании выражений этих типов и работе с ними см. в разделе [выражения типов](xref:microsoft.quantum.guide.expressions).

Обратите внимание, что Q# является *строго типизированным* языком, так что Аккуратное использование этих типов может помочь компилятору обеспечить строгую гарантию для Q# программ во время компиляции.
Чтобы обеспечить максимально возможную гарантию, преобразования между типами в Q# должны быть явными с помощью вызовов функций, которые выражают это преобразование. 
Q# предоставляет множество таких функций, как часть <xref:Microsoft.Quantum.Convert> пространства имен.
...
Q#Язык предоставляет следующие типы- *примитивы* , которые можно использовать непосредственно в Q# программах. Эти примитивные типы также можно использовать для создания новых типов.
...
- `Unit`Тип может иметь только одно значение, `()` . 
  Используйте этот тип, чтобы указать, что Q# функция или операция не возвращает никакой информации. 
...
* Для любого допустимого Q# типа существует тип, представляющий массив значений этого типа.
...
* Значение массива может быть записано в Q# исходном коде с помощью квадратных скобок вокруг элементов массива, как в `[PauliI, PauliX, PauliY, PauliZ]` .
...
Кортежи — это мощная концепция, используемая в процессе объединения Q# значений в одно значение, что упрощает их передачу.
...
* Каждый Q# исходный файл может объявлять любое количество определяемых пользователем типов.
...
Q#Файл может определять новый именованный тип, содержащий одно значение любого допустимого типа.
...
Помимо анонимных элементов, определяемые пользователем типы также поддерживают *именованные элементы* Q# версии 0,7 или более поздней. Например, можно присвоить элементам имя `Real` Double, представляющей реальную часть комплексного числа и `Imag` для мнимой части: 
...
При возврате к примеру `Complex` , один из них мог бы также определить полярную координату репресенатион в качестве определяемого пользователем типа:

```qsharp
newtype ComplexPolar = (Magnitude : Double, Argument : Double);
```

Хотя оба типа `Complex` и `ComplexPolar` оба имеют базовый тип `(Double, Double)` , эти два типа полностью несовместимы в, что Q# минимизирует риск случайного вызова сложной математической функции с полярными координатами и наоборот.
...
Дополнительные сведения о операторе Unwrap см. [в разделе выражения типа Q# в ](xref:microsoft.quantum.guide.expressions).
...
* Все Q# вызываемые команды принимают одно значение в качестве входных данных и возвращают одно значение в качестве выходных данных.
...
> [!NOTE]
> В этой статье обсуждается только, как операторов изменяет подпись операции. Дополнительные сведения о операторов и операциях см. [в разделе операции и Q# функции в ](xref:microsoft.quantum.guide.operationsfunctions). 
...
### <a name="type-parameterized-functions-and-operations"></a>Функции и операции Type-Parameterized
...
Дополнительные сведения и сведения о том, как определять вызываемые параметрами типа, см. [в разделе операции и Q# функции в ](xref:microsoft.quantum.guide.operationsfunctions#generic-type-parameterized-callables).
...
Теперь, когда вы видели все типы Q# , составляющие язык, см. раздел [Expression types в Q# ](xref:microsoft.quantum.guide.expressions) , чтобы научиться создавать и манипулировать выражениями этих различных типов.
