...
Аджоинтсие операций с тактами крайне важно для многих аспектов тактовых вычислений. Пример одной из таких ситуаций, обсуждаемой наряду с полезным Q# методом программирования, см. в разделе [Flow Control: лиц](xref:microsoft.quantum.guide.controlflow#conjugations). Управляемая версия операции — это новая операция, которая фактически применяет базовую операцию только в том случае, если все элементы управления Кубитс находятся в указанном состоянии.
...
## <a name="callables-as-first-class-values"></a>Вызываемые как значения First-Class
