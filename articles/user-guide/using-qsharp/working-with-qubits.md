...
В этой статье рассматривается использование и работа с Кубитс в программе Q #. 
...
Поэтому необходимо указать Q #, чтобы *выделить* Кубитс для использования в определенном блоке инструкций.
...
Пример использования в Q # см. в подразделе [Кубитс example (пример](#borrowing-qubits-example) ) в этой статье или в документе [*с использованием 2N + 2 Кубитс с Тоффоли модульного умножения*](https://arxiv.org/abs/1611.07995) (Ханер, роеттелер и своре 2017) для алгоритма, использующего заимствованные Кубитс.
...
В этой статье рассматриваются несколько полезных операций Q #, которые можно использовать для взаимодействия с Кубитс.
...
Во-первых, операторы Single-кубит Паули $X $, $Y $ и $Z $ представлены в Q # встроенными операциями [`X`](xref:microsoft.quantum.intrinsic.x) , [`Y`](xref:microsoft.quantum.intrinsic.y) и [`Z`](xref:microsoft.quantum.intrinsic.z) , каждый из которых имеет тип `(Qubit => Unit is Adj + Ctl)` .
...
Кроме того, можно подготовить такие состояния, как $ \кет{+} = \лефт (\кет {0} + \кет {1} \ригхт)/\скрт {2} $ и $ \кет {-} = \лефт (\кет {0} -\кет {1} \ригхт)/\скрт {2} $, с помощью хадамард Transform $H $, которое представлено в Q # встроенной операцией [`H`](xref:microsoft.quantum.intrinsic.h) (также типа (кубит => единица — прилагательные и CTL) "):
