...
---
# <a name="jordan-wigner-representation"></a>Представление Jordan-Wigner
...
Основным преимуществом представления Jordan-Wigner является его простота.

Представление Jordan-Wigner прямо вперед к производному.
...
К счастью, строки $Z $ Operators (называемые Jordan-Wigner строками) отменяются после выполнения этой подстановки.
После истечения этого (и повторного вызова этого $X _jY_j = iZ_j $) у нас есть \бегин{екуатион} n_j = a ^ \ dagger_j a_j = \фрак{(1-Z_j)} {2} .
\енд{екуатион}


## <a name="constructing-hamiltonians-in-jordan-wigner-representation"></a>Создание Хамилтонианс в представлении Jordan-Wigner

После вызова представления Jordan-Wigner, в котором Хамилтонианся сумма операторов Паули, выполняется Прямая переадресация.
