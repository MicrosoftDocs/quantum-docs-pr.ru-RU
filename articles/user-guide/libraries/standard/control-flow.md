...
В Q # мы можем использовать <xref:microsoft.quantum.arrays.indexrange> , чтобы представить это как `for` цикл по регистру:
...
Сигнатура `DecomposeIntoTimeStepsCA` соответствует общему шаблону в Q #, где коллекции, которые могут быть включены в массивы, или что-то, какие элементы вычислений в режиме реального времени представлены кортежами, первыми элементами которых являются `Int` значения, указывающие на их длину.
...
Таким образом, наша новая функция выглядит и похожа на ту `Controlled` , что мы можем легко определить мощные новые конструкции потока управления, используя Q # и Canon вместе:
