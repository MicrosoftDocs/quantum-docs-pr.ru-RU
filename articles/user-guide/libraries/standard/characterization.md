...
Такая маскировка состояния такта от пользователя отражается на том, что не Q# предоставляет или даже не определяет состояние, которое *относится* к тактовым программам.
...
Как описано в разделе по Oracle в [структурах данных](xref:microsoft.quantum.libraries.data-structures), Q# Canon моделирует такие операции с помощью <xref:Microsoft.Quantum.Oracles.DiscreteOracle> определяемого пользователем типа, определенного типом кортежа `((Int, Qubit[]) => Unit : Adjoint, Controlled)` .
...
Мы Подчеркните это, кратко описывая оценку итеративного этапа на теоретическом уровне в формальном механизме оценки параметров Байеса, прежде чем продолжать описывать статистические алгоритмы, предоставляемые в Q# Canon для решения этой классической проблемы вывода.
...
В частности, один шаг PE выполняет следующее неединое преобразование состояния \бегин{алигн} \ sum_j \Скрт{\пр (\фи \_ j)} \кет{\фи \_ j} \мапсто \сум \_ ж\фрак {\ sqrt {\ PR (\фи \_ j)} \скрт{\пр (\текст{ресулт} | \Phi \_ j)} \Ket{\Phi \_ j}} {\sqrt{\Pr (\Phi \_ j) \sum \_ k \Pr (\Text{Result} | \Phi \_ k)}}.
...
> [!TIP]
> Дополнительные сведения об оценке этапа Байеса на практике см. в примере [**фасистиматион**](https://github.com/microsoft/Quantum/tree/main/samples/characterization/phase-estimation) .
...
Каждая операция оценки этапа, обеспечиваемая Q# Canon, принимает другой набор входных данных, обменяя качество, которое мы хотим используть для окончательной оценки $ \хат{\фи} $.
