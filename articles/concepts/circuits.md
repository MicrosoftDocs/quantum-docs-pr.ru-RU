...
— Это [хадамард](xref:microsoft.quantum.primitive.h) -вентиль, действующая на одном кубит регистре.
...
Действие одномерного управляемого генератора, обозначенное как $ \Ламбда (G) $, где значение одного кубита управляет применением $G $, можно понять, просмотрев следующий пример входного состояния продукта $ \Ламбда (G) (\алфа \кет{0} + \бета \кет{1}) \кет{\пси} = \алфа \кет{0} \кет{\пси} + \бета \кет{1} Г\кет {\ PSI} $.
...
В особых случаях, где $G = X $ и $G = Z $, мы представляем следующую нотацию для описания управляемой версии шлюзов (Обратите внимание, что шлюз управляемого X является [$CNOTным шлюзом](xref:microsoft.quantum.primitive.cnot)):
...
Q # реализует [оператор Measure](xref:microsoft.quantum.primitive.measure) для этой цели.
