...
В качестве примера того, как можно построить единые преобразования из этих примитивов, три преобразования, представленные в БЛОЧ шарик выше, соответствуют последовательности Gate $ \бегин{бматрикс} 1 \\\\ 0 \енд{бматрикс} \мапсто хз \бегин{бматрикс} 1 \\\\ 0 \енд{бматрикс} = \бегин{бматрикс} 0 \\\\ 1 \end{bmatrix} $.
...
\бегин{алигн *} & R_z (\сета) = e ^ {-И\сета z/2} = \бегин{бматрикс} e ^ {-i \ тета/2} & 0\\\\ 0 & e ^ {i \ тета/2} \енд{бматрикс}, \\\\ & R_x (\сета) = e ^ {И\сета x/2} = HR_z (\сета) H = \бегин{бматрикс} \cos (\ тета/2) &-i\sin (\ тета/2)\\\\-i\sin (\ тета/2) & \cos (\ тета/2) \end{bmatrix}, \\\\ & R_y (\theta) = e ^ {I\theta y/2} = SHR_z (\theta) HS ^ \dagger = \begin{bmatrix} \cos (\ тета/2) &-\sin (\ тета/2)\\\\ \син (\ тета/2) & \кос (\ тета/2) \енд{бматрикс}. \енд{алигн*}
