...
Хотя это тривиальный пример (так как <xref:microsoft.quantum.intrinsic.h> операция является самостоятельным), вы можете увидеть, как это становится неполезным для более сложных операций кубит.
...
$ $ H = \фрак {1} {\скрт {2} } \бегин{бматрикс} 1 & 1 \\ \\ 1 &-1 \енд{бматрикс}, \ккуад S = \бегин{бматрикс} 1 & 0 \\ \\ 0 & i \енд{бматрикс} = T ^ 2, \ккуад X = \begin{bmatrix} 0 &1 \\ \\ 1& 0 \end{bmatrix} = HT ^, $ $

$ $ Y = \бегин{бматрикс} 0 &-i \\ \\ & 0 \Енд{бматрикс} = T ^ 2HT ^ 4 HT ^ 6, \ккуад Z = \begin{bmatrix}1&0 \\ \\ 0&-1 \енд{бматрикс} = T ^ 4.
$$
