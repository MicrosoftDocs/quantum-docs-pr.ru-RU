---
title: 'Структуры данных в стандартных библиотеках Q #'
description: 'Сведения о структурах данных, Oracle и динамических генераторах в стандартных библиотеках Microsoft Q #.'
...
Аналогичным образом мы видели в предыдущем разделе Типы формы `(Int, Int -> T)` могут быть полезны для представления коллекций произвольного доступа, поэтому функция <xref:microsoft.quantum.arrays.lookupfunction> предоставляет удобный способ создания таких типов из типов массивов.
...
Этот метод является незаменимым при моделировании такта, так как для многих физических проблем в химия и материальных науки эти еиженвалуес приводят к силыу в тактовых системах, которая предоставляет ценные сведения о схемах этапов материалы и радиосвязи для молекул.
...
В таких случаях подпрограммы, с которыми мы будем взаимодействовать, для того, чтобы изучить фиксированное значение $ \сета $ для шлюза, — $ $ \бегин{алигн} U & = R_z (\сета) \\\\ & = \бегин{бматрикс} e ^ {-i \сета/2} & 0 \\\\ 0 & e ^ {i \ тета/2} \енд{бматрикс}.
\енд{алигн} $ $
...
Первый элемент `Int` кортежа содержит количество терминов $d $ в Хамилтониан, а второй элемент `(Int -> GeneratorIndex)` является функцией, которая сопоставляет целочисленный индекс в $\{0, 1,..., d-1\}$ с `GeneratorIndex`ным пользователем типом, который однозначно определяет каждый примитивный термин в Хамилтониан. Обратите внимание, что путем выражения коллекции терминов в Хамилтониан в качестве функции, а не `GeneratorIndex[]`массива, это позволяет выполнять вычисления `GeneratorIndex`, которые особенно полезны при описании Хамилтонианс с большим количеством терминов.
...
$ $ \бегин{алигн} H & = \сум ^ {d-1} _ {j = 0} a_j H_j, \енд{алигн} $ $, где каждый $ \хат H_j $ теперь взят из группы Паули. Для таких систем мы предоставляем `PauliEvolutionSet()` типа `EvolutionSet`, который определяет соглашение о том, как элемент группы Паули и коэффициент можно идентифицировать с помощью `GeneratorIndex`, который имеет следующую сигнатуру.
