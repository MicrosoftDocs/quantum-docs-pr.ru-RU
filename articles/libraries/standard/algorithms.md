...
Для приблизительного Кфт требуется операция <xref:microsoft.quantum.primitive.rfrac> $Z $-вращение, а также <xref:microsoft.quantum.intrinsic.h>ная операция.
Предполагается, что входные и выходные данные кодируются в кодировке с обратным порядком байтов (самый маленький бит/кубит находится слева, то же, что и [Сисакет запись](xref:microsoft.quantum.concepts.dirac)).
...
Как и в случае с арифметическими вычислениями, эта роль также незаменимым в тактовых вычислениях.  Такие алгоритмы, как алгоритм факторинга Шор, методы моделирования тактов, а также многие алгоритмы оракулар полагаются на согласованные арифметические операции.  Большинство подходов к арифметической построению на основе цепей тактовой Adder.  Простейший Adder принимает классическое входное $b $ и добавляет значение в состояние такта, содержащее целое число $ \кет{а} $.  С помощью математического выражения Adder (который, по-вашему, стоит $ \Операторнаме{адд} (b) $ для классического ввода $b $) имеет свойство, которое

$ $ \Операторнаме{адд} (b) \кет{а} = \кет{а + b}.
$ $ Этот базовый канал Adder является более инкрементным, чем Adder.
Его можно преобразовать в Adder с двумя входными тактами через $ $ \Операторнаме{адд}\кет{а}\кет{б} = \кет{а}\кет{а + b}, $ $ с помощью $n $ controlled Applications методах формы \бегин{алигн} \Операторнаме{адд} \ket{a} \ket{b} & = \Lambda\_{a\_0} \лефт (\Операторнаме{адд} (1) \ригхт) \Ламбда\_{a\_1} \лефт (\Операторнаме{адд} (2) \ригхт) \Ламбда\_{a\_2} \лефт (\operatorname{Add} (4) \right) \cdots \Lambda\_{a\_{n-1}} \left (\ операторнаме {Add} ({{n-1}}) \ригхт) \кет{а}\кет{б} \\\\ & = \кет{а} \кет{б + a}, \енд{алигн} для $n $-разрядных целых чисел $a $ и $b $ и сложения по модулю $2 ^ n $.  Помните, что нотация $ \Ламбда\_x (A) $ относится для любой операции $A $ к контролируемой версии этой операции с элементом управления кубит $x $ AS.

Аналогичным образом, классический контролируемый способ умножения (Модульная форма, которая необходима для алгоритма факторинга Шор) может быть выполнена с помощью аналогичной серии контролируемых дополнений: \бегин{алигн} \Операторнаме{мулт} (a) \кет{КС}\кет{б} & = \Ламбда\_{x\_0} \лефт (\Операторнаме{адд} (2 ^ 0 a) \ригхт) \Ламбда\_{a\_1} \лефт (\Операторнаме{адд} (2 ^ 1a) \ригхт) \Ламбда\_{\_2} \лефт (\operatorname{Add} (2 ^ 2 a) \right) \cdots \Lambda\_{x\_{ n-1}} \лефт (\Операторнаме{адд} ({2 ^ {n-1}} a) \ригхт) \кет{КС}\кет{б} \\\\ & = \кет{КС}\кет{б + AX}.
...
$ $ Если мы определяем $ $ \кет{\фи\_k (a)} = \фрак{1}{\скрт{2}} \лефт (\кет{0} + e ^ {i2\pi a/2 ^ k} \кет{1} \ригхт), $ $ после некоторой рекурсии можно увидеть, что $ $ \Операторнаме{Кфт}\кет{а} = \кет{\фи\_1 (a)} \отимес \cdots \ отимес \кет{\фи\_n (a)}.
...
### <a name="quantum-phase-estimation"></a>Оценка фазы такта ###
...
Предположим, что $V $ является управляемым приложением $U $, таким, что \бегин{алигн} V (\кет{0} \отимес \кет{\фи}) & = \кет{0} \отимес \кет{\фи} \\\\ \текстрм{и} V (\кет{1} \отимес \кет{\фи}) & = e ^ {i \Phi} \ket{1}  \отимес \кет{\фи}.
\енд{алигн}, по линейной шкале, \бегин{алигн} V (\кет{+} \отимес \кет{\фи}) & = \фрак{(\кет{0} \отимес \кет{\фи}) + e ^ {i \фи} (\ket{1} \otimes \ket{\Phi})} {\sqrt{2}}.
\енд{алигн} мы можем собираются термины, чтобы найти \бегин{алигн} V (\кет{+} \отимес \кет{\фи}) & = \фрак{\кет{0} + e ^ {i \фи} \кет{1}} {\скрт{2}} \отимес \ket{\Phi} \\\\ & = (R_1 (\Phi) \ket{+}) \otimes \ket{\Phi} , \енд{алигн}, где $R _1 $ является единым, применяемым операцией <xref:microsoft.quantum.intrinsic.r1>.
...
Продолжая таким образом, можно получить регистр в формате \бегин{алигн} \кет{\пси} & = \sum_{j = 0} ^ n R_1 (2 ^ j \фи) \кет{+} \\\\ & \пропто \bigotimes_{j = 0} ^ {n} \лефт (\кет{0} + \експ (i 2 ^ {j} \фи) \кет{1}\ригхт) @no__ t_4_ \\ & \пропто \sum_{k = 0} ^ {2 ^ n-1} \експ (i \фи k) \кет{к} \енд{алигн}, где $n $ — число требуемых разрядов точности, и где мы использовали ${} \пропто {}$, чтобы указать, что мы отменяли коэффициент нормализации $ 1/\sqrt{2 ^ n} $.

Если предполагается, что $ \фи = 2 \пи p/2 ^ k $ для целого числа $p $, то мы понимаем это как $ \кет{\пси} = \Операторнаме{Кфт} \ket{p_0 p_1 \дотс p_n} $, где $p _j $ — $j ^ {\текстрм{с}} $ bit $2 \пи \фи $.
