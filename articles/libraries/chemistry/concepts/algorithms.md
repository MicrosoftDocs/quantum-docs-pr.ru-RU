...
Позвольте $r $ быть числом шагов, предпринятых в ходе эволюции, так что каждый раз, когда шаг выполняется в течение времени $t/r $. Затем у нас есть $ $ e ^ {-i \ sum_ {j = 1} ^ m H_j t} = \лефт (\ prod_ {j = 1} ^ m e ^ {-iH_j t/r} \ right) ^ r + O (m ^ 2 t ^ 2/r), $ $ что означает, что если $r $ масштабируется как $m ^ 2 t ^ 2/\ Эпсилон $, то ошибку можно сделать не более $ \епсилон $ для любого $ \епсилон > 0 $.
...
Даже более высокие формулы, в частности ($ 2000 $) TH-Order для $k > 0 $, можно формировать рекурсивно: $ $ U_  (t) = [U_ {2000-2} (s_k\~ t)] ^ 2 U_ {2000-2} ([1-4s_k] t) [U_ {2000 – 2} (s_k\~ t)] ^ 2 = e ^ {-ИХТ} + O ((m t) ^ {2000 + 1}/r ^ ), $ $ Where $s _k = (4-4 ^ {1/(2000-1)}) ^{-1}$.
