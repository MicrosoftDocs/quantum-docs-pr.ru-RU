...
Для отправки неизвестного состояния такта (которое мы будем называть "__сообщением__") из кубит в одном месте кубит в другом месте (мы будем называть эти Кубитс как "__здесь__"__и "in")__ . соответственно). Мы можем представить наше __сообщение__ в виде вектора с помощью нотации Дирак: 
...
$ $ \фрак{\алфа}{\скрт{2}} (\фрак{1}{\скрт{2}} (\кет{0} + \кет{1})) \кет{00} + \фрак{\алфа}{\скрт{2}} (\фрак{1}{\скрт{2}} (\кет{0} + \кет{1})) \ket{11} + \frac{\beta}{ \скрт{2}} (\фрак{1}{\скрт{2}} (\кет{0}-\кет{1})) \кет{10} + \фрак{\бета}{\скрт{2}} (\фрак{1}{\скрт{2}} (\кет{0}-\кет{1})) \кет{01} $ $
...
$ $ \фрак{\алфа}{2}(\кет{0} + \кет{1}) \кет{00} + \фрак{\алфа}{2}(\кет{0} + \кет{1}) \кет{11} + \фрак{\бета}{2}(\кет{0}-\кет{1}) \кет{10} + \фрак{\бета}{2}(\кет{0}-\кет{1}) \кет{01} $ $
...
$ $ \фрак{1}{2}\биг [\алфа (\кет{0} + \кет{1}) \кет{00} + \алфа (\кет{0} + \кет{1}) \кет{11} + \бета (\кет{0}-\кет{1}) \кет{10} + \бета (\кет{0}-\кет{1}) \кет{01}\биг] $ $
...
### <a name="step-1-create-an-entangled-state"></a>Шаг 1. Создание состояния запутанными
Затем можно создать пару запутанными между `here` и `there` с помощью операций @"microsoft.quantum.primitive.h" и @"microsoft.quantum.primitive.cnot":
...
Наконец, мы используем @"microsoft.quantum.primitive.m" для выполнения измерений и выполнения необходимых операций Gate для получения требуемого состояния, как показано в инструкциях `if`:
