...
Операция типа `(Qubit => Unit)` не поддерживает ни `Adjoint`, ни `Controlled` функтор. Если нам нужно указать, что операция этого типа должна поддерживать, например `Adjoint` функтор, нам нужно объявить ее как аджоинтабле. Это делается с помощью аннотации, `is Adj` типу. Аналогичным образом `(Qubit => Unit is Ctl)` указывает, что операция этого типа поддерживает `Controlled` функтор. Мы рассмотрим это Подробнее, когда мы обсудим [типы в Q #] (xref: Microsoft. тактов. Language. Type-Model), как правило, в общем.
