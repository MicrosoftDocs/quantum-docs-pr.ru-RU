...
*Дата выпуска: Январь 26th, 2021*

- Упрощенное выделение кубит, предоставляя более удобный синтаксис для выделения Кубитс, [см. сведения в Q# репозитории языка](https://github.com/microsoft/qsharp-language/blob/main/Approved/1-implicitly-scoped-qubit-allocation.md).
- Создан QDK-Python репозиторий, включающий в себя `azure-quantum` клиент Python для отправки заданий оптимизации на основе такта в службу тактовой передачи Azure, а также, в `qdk` том числе `qdk.chemistry` , удобный уровень на базе Python для Q# библиотеки химия, который включает визуализацию молекулярное и функциональные возможности для создания входных файлов для нескольких химия пакетов, таких как Нвчем, Psi4 и опенмолкас.
- Круглые скобки теперь являются необязательными для типов операций и функций `if` , а `elif` операторы, `while` и `until` . Скобки для `for` `use` операторов и не `borrow` рекомендуются.
...
*Дата выпуска: Октябрь 27, 2020*

Этот выпуск включает следующие обновления:

- Теперь Оценка ресурсов выдает одновременно недостижимые оценки глубины и ширины в дополнение к числу кубит. Подробные сведения см. [здесь](xref:microsoft.quantum.machines.resources-estimator#metrics-reported).

См. полный список закрытых вытягивание для [библиотек](https://github.com/Microsoft/QuantumLibraries/pulls?q=is%3Apr+closed%3A2020-09-25..2020-10-22), [компилятора](https://github.com/microsoft/qsharp-compiler/pulls?q=is%3Apr+closed%3A2020-09-25..2020-10-22), [среды выполнения](https://github.com/microsoft/qsharp-runtime/pulls?q=is%3Apr+closed%3A2020-09-25..2020-10-22), [примеров](https://github.com/Microsoft/Quantum/pulls?q=is%3Apr+closed%3A2020-09-25..2020-10-22), [I Q# ](https://github.com/microsoft/iqsharp/pulls?q=is%3Apr+closed%3A2020-09-25..2020-10-22) и [Катас](https://github.com/microsoft/QuantumKatas/pulls?q=is%3Apr+closed%3A2020-09-25..2020-10-22).
...
- Обновлен пример Хидденшифт.
- Добавлен пример для решения Судоку с алгоритмом Гровер (внешний вклад)
...
- Новая поддержка Q# приложений, для которых больше не требуется файл узла C# или Python. Дополнительные сведения о начале работы с Q# приложениями см. [здесь](xref:microsoft.quantum.install.standalone).
...
> [!NOTE]
> Q# приложения, использующие новый [`@EntryPoint()`](xref:Microsoft.Quantum.Core.EntryPoint) атрибут, в настоящее время не могут вызываться из программ хоста Python или .NET.
...
- Новый атрибут теста для Q# модульного тестирования. см. обновленную документацию по API [здесь](xref:Microsoft.Quantum.Diagnostics.Test) и [](xref:microsoft.quantum.guide.testingdebugging) обновленное руководство по тестированию & отладки
...
2. В командной строке выполните команду, `dotnet clean` чтобы удалить все существующие двоичные файлы и промежуточный файл.
3. В текстовом редакторе измените CSPROJ-файл, указав для всех упоминаний Microsoft.Quantum `PackageReference` версию 0.6.1904, а имя пакета Microsoft.Quantum.Canon измените на Microsoft.Quantum.Standard, как показано ниже:
    ...
    ```
4. В командной строке выполните следующую команду: `dotnet msbuild`  
...
Этот выпуск включает исправления ошибок и функции для проблем, о которых сообщили сообщество.
...
Этот выпуск включает исправления ошибок для решения проблем, о которых сообщило сообщество.
...
- `--filter`Параметр для не `dotnet test` работает правильно для тестов, написанных в Q# .
  В результате отдельные модульные тесты не могут выполняться в Visual Studio Code; `dotnet test` для повторного выполнения всех тестов рекомендуется использовать команду из командной строки.
