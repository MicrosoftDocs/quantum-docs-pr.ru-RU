...
| Мера | Выполняется измерение такта, и результат возвращается пользователю. |
...
Более того, Q# точки входа могут быть точками входа для всего приложения (например, в Q# автономных исполняемых программах) или интерфейсом между Q# программой и основной программой для приложения (т. е. при использовании Q# с Python или .NET), в результате чего имя «main» может быть ошибочным при применении к Q# точке входа.
...
### <a name="user-defined-type-named-items"></a>User-Defined именованные элементы типа ###
...
> [Справочник по синтаксису комментариев документации](xref:microsoft.quantum.qsharp.comments#documentation-comments).
