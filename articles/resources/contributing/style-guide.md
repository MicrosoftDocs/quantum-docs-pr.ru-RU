...
# <a name="guidance"></a>[Руководство](#tab/guidance)
...
| ☒ | <s>`operation search_oracle`</s> | Использование `snake_case` нотации Контравенес Q #. |
...
При определении точки входа в программу Q # компилятор Q # распознает [ `@EntryPoint()` атрибут](xref:microsoft.quantum.core.entrypoint) , а не требует, чтобы точки входа имели определенное имя (например `main` ,, `Main` или `__main__` ).
То есть с точки зрения разработчика Q # точки входа являются обычными операциями, заметками которых является `@EntryPoint()` .
Более того, Q # точки входа могут быть точками входа для всего приложения (например, в Q # standalone исполняемые файлы) или интерфейсом между программой Q # и основной программой для приложения (т. е. при использовании Q # с Python или .NET), чтобы имя «main» могло быть ошибочным при применении к точке входа Q #.
...
В качестве конкретного примера, контролируемые с помощью управляемой операции переключения и удвоения часто называются операциями "Фредкин" и "Тоффоли" в учебных литературах, но они определяются в Q # преимущественно как `CSWAP` и `CCNOT` .
...
Этот приоритет особенно важен в тех случаях, когда необходимо всегда использовать собственные существительные: Q # соответствует традицию, заданному многими классическими языками, и ссылается на `Bool` типы в ссылке на логическую логику, которая в свою очередь имеет имя, соблюдающая bool.
Несколько концепций такта аналогичным образом именуются, включая `Pauli` тип, встроенный в язык Q #.
...
- Если функция, операция или определяемый пользователем тип не является частью общедоступного API для библиотеки или программы Q #, убедитесь, что она помечена как внутренняя, поместив `internal` ключевое слово перед `function` `operation` `newtype` объявлением, или.
