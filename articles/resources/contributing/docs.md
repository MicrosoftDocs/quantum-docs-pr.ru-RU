...
Эта команда импортирует строки 4 – 8 [ `Game.qs` файла из `chsh-game` примера](https://github.com/microsoft/Quantum/blob/master/samples/algorithms/chsh-game/Game.qs), помечая их как код Q # для выделения синтаксиса.
...
- Формат каждого комментария документации должен совпадать с тем, что компилятор Q # ожидает, чтобы ваша документация отображалась правильно. Некоторые разделы, например, `/// # Remarks` позволяют использовать произвольное содержимое, а такие разделы, как, `/// # See Also` являются более узкими.
