...
Автор: брадбен MS. author: v-бенбра MS. Дата: 9/1/2020 МС. раздел: эталонный UID: Microsoft. тактов. Глоссарий No-Loc:
- "Q#"
- "$$v"
- "$$"
- "$$"
...
[Операция](xref:microsoft.quantum.glossary#operation) или [функция](xref:microsoft.quantum.glossary#function) [ Q# языка](https://github.com/microsoft/qsharp-language/tree/main/Specifications/Language#q-language).
Дополнительные сведения см. в разделе [ Q# программы](xref:microsoft.quantum.guide.programs) .
...
[Операция](xref:microsoft.quantum.glossary#operation) -такт, которая принимает один или несколько [Кубитс](xref:microsoft.quantum.glossary#qubit) как созидателями для целевой операции. Дополнительные сведения см. в разделе [функтор Application](xref:microsoft.quantum.qsharp.functorapplication#functor-application).
...
Тип подпрограммы на Q# языке, который является исключительно детерминированным. Хотя функции используются в алгоритмах такта, они не могут действовать в [операциях](xref:microsoft.quantum.glossary#operation) [Кубитс](xref:microsoft.quantum.glossary#qubit) или Call. Дополнительные сведения см. в разделе [ Q# программы](xref:microsoft.quantum.guide.programs) .

## <a name="gate"></a>Frame

Устаревший термин для некоторых внутренних [операций](xref:microsoft.quantum.glossary#operation)такта, основанный на концепции классических логик шлюзов. [Тактовая цепь](xref:microsoft.quantum.glossary#quantum-circuit-diagram) — это сеть шлюзов, основанная на аналогичной концепции классических логических цепей.
...
Базовая единица вычислений тактов в Q# . Он примерно эквивалентен функции в C, C++ или Python или статическом методе в C# или Java. Дополнительные сведения см. в разделе [ Q# программы](xref:microsoft.quantum.guide.programs).
...
## <a name="partial-application"></a>Применение частично выполненной строки

Вызов [функции](xref:microsoft.quantum.glossary#function) или [операции](xref:microsoft.quantum.glossary#operation) без всех необходимых входных данных. Он возвращает новый [вызываемый](xref:microsoft.quantum.glossary#callable) объект, которому требуются только отсутствующие параметры (обозначенные символом подчеркивания), которые должны быть предоставлены в будущем приложении. Дополнительные сведения см. в разделе [частичное применение](xref:microsoft.quantum.qsharp.partialapplication).

## <a name="pauli-operators"></a>Операторы Паули

Набор одноединых матриц 3 2 x 2, известных как `X` операции, так `Y` и `Z` такты. Матрица идентификации, $ I $ , часто также включается в набор.  $1 0 0 = \begin{bmatrix} & \\\\ & 1 \end{bmatrix} $ , $ X = \begin{bmatrix} 0 & 1 \\\\ 1 & 0 \end{bmatrix} $ , $ Y = \begin{bmatrix} 0 & -i 0 \\\\ & \end{bmatrix} $ , $ Z 1 0 = \begin{bmatrix} & \\\\ & -1 \end{bmatrix} $ .   Дополнительные сведения см. в статье [Single-кубит Operations](xref:microsoft.quantum.concepts.qubit#single-qubit-operations).

## <a name="quantum-circuit-diagram"></a>Схема тактовой цепи

Метод для графического представления последовательности [шлюзов](xref:microsoft.quantum.glossary#gate) для простых тактовых программ, например 
...
Концепция часто используется в алгоритмах такта, состоящих из многократно применяемых вычислений до тех пор, пока не будет удовлетворено определенное условие. Если условие не удовлетворено, часто требуется указать адресную привязку перед повторной попыткой, введя следующую итерацию. Дополнительные сведения см. в разделе с [ Q# руководством пользователя](xref:microsoft.quantum.guide) .
...
Пользовательский тип, который может содержать один или несколько именованных или анонимных элементов. Дополнительные сведения см. в разделе [объявления типов] Microsoft. тактов. кшарп. типедекларатионс # Type-Declarations).
